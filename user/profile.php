...
// Fetch user data from the database (ONLY the logged-in user)
...
if ($user_result->num_rows > 0) {
    $user_row = $user_result->fetch_assoc(); // <-- don't call this $user_data later again
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_profile'])) {
        ...
        $profile_pic_current = $user_row['profile_pic']; // may be 'default-profile.jpg' or a path or filename
...
while ($row = $user_query->fetch_assoc()) {
    $users[(int)$row['id']] = ['name' => $row['name'], 'points' => 0];
}

$nameToIdMap = [];
foreach ($users as $uid => $urow) {
...
if ($result) {
    while ($sub = $result->fetch_assoc()) {
        ...
        $pts          = (int)$sub['points'];
        $team_members_json = $sub['team_members'];

        if (isset($users[$submitter_id])) {
            $users[$submitter_id]['points'] += $pts;
        }

        // Credit teammates (by name — consider migrating to user IDs later)
        $team_members = json_decode($team_members_json, true);
        if (is_array($team_members)) {
            foreach ($team_members as $memberName) {
                if (isset($nameToIdMap[$memberName])) {
                    $memberId = (int)$nameToIdMap[$memberName];
                    if ($memberId !== $submitter_id) {
                        $users[$memberId]['points'] += $pts;
                    }
                }
            }
        }
...
// Total points for this user (even if zero)
$total_points = isset($users[$user_id]) ? (int)$users[$user_id]['points'] : 0;

// Compute rank only if the user has points; otherwise rank = 0
$user_rank = 0;
if ($total_points > 0) {
    // Sort by points desc
    $sorted = $users;
    uasort($sorted, fn($a, $b) => $b['points'] <=> $a['points']);

    $rank = 1;
    foreach ($sorted as $uid => $row) {
        if ((int)$row['points'] > 0) {
            if ((int)$uid === $user_id) {
                $user_rank = $rank;
                break;
...
<head>
    ...
    <title>GreenCredit - User Profile</title>
        ...
        <div class="col-md-4">
            ...
            <div class="profile-section text-center">
                ...
                <div class="d-flex justify-content-center gap-2 mb-3">
                    <span class="rank-badge"><i class="fas fa-trophy me-2"></i>Rank #<?= (int)$user_rank; ?></span>
                    <span class="rank-badge" style="background-color:#4a90e2;"><i class="fas fa-star me-2"></i><?= number_format((int)$total_points); ?> pts</span>
                </div>
